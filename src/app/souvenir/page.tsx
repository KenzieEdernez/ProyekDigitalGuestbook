...
export default function SouvenirPage() {
  ...
  const transactions = allGuests
    ...
    .filter(
      (g) =>
        !searchLog ||
        g.name.toLowerCase().includes(searchLog.toLowerCase())
    )
    .slice(0, 10);

  const totalClaimed = allGuests.filter(
    (g) => g.status === "souvenir_claimed"
  ).length;
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {inventory.map((item) => (
                <div key={item.name} className="card-premium p-5">
                  ...
                  <span className={`badge mt-2 ${item.statusColor}`}>
                    {item.status}
                  </span>
                </div>
              ))}
            ...
            <div className="card-premium overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead className="bg-stone-50 text-left">
                  <tr>
                    <th className="px-6 py-3 text-xs font-semibold uppercase text-stone-400">
                      Waktu
                    </th>
                    <th className="px-6 py-3 text-xs font-semibold uppercase text-stone-400">
                      Tamu
                  ...
                  {transactions.map((g) => (
                    <tr key={g.id} className="border-t border-stone-50">
                      <td className="px-6 py-3 text-stone-500">
                        {new Date(g.souvenir_claimed_at!).toLocaleTimeString(
                          "id-ID",
