...
export default function GuestListPage() {
  ...
  const exportPrintableReport = async () => {
    let wishes: Array<{
      guest_name: string;
      message: string;
    ...
    const wishRows = wishes
      .map(
        (wish, index) => `
          <tr>
            <td>${index + 1}</td>
            <td><strong>${escapeHtml(wish.guest_name)}</strong></td>
            ...
            <td>${escapeHtml(formatDateTime(wish.created_at))}</td>
          </tr>
        `
      )
      .join("");
      ...
      <html>
        ...
        <body>
          <main class="page">
            ...
            <h2 style="margin:28px 0 12px;font-family:Georgia,serif;font-size:22px;">Guest Wishes</h2>
            <table>
              <thead>
                <tr>
                  <th>No</th>
              ...
              <tbody>${wishRows || `<tr><td colspan="5" class="center">No wishes yet</td></tr>`}</tbody>
            </table>
